...
{
    public:
    void findPre(Node* root, Node*& pre, Node*& suc, int data)
    {
        if(root==NULL) return;
        if(root->key <data && pre->key <root->key){
            pre = root;
        }
        if(root->key > data && suc->key > root->key){
            suc = root;
        }
        findPre(root->left, pre, suc, data);
        findPre(root->right, pre, suc, data);
        return;
    }
